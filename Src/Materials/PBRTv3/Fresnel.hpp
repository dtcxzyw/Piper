...
private:
    float mEtaI, mEtaT;
public:
    DEVICE FresnelDielectric(float etaI, float etaT) :mEtaI(etaI), mEtaT(etaT) {}
...
public:
    DEVICE FresnelConductor(const Spectrum &etaI, const Spectrum &etaT,
        const Spectrum &k) :mEtaI(etaI), mEtaT(etaT), mK(k) {}
    DEVICE Spectrum eval(float cosThetaI) const;
};
...
private:
    ...
    bool mConductor;
public:
    DEVICE Fresnel(const FresnelConductor &con) :mConductor(true), mContent(con) {}
    DEVICE Fresnel(const FresnelDielectric &die) : mConductor(false), mContent(die) {}
    DEVICE Spectrum eval(float cosThetaI) const {
