...
namespace TrowbridgeReitzDistribution {
    INLINEDEVICE float roughnessToAlpha(float roughness) {
    ...
    INLINEDEVICE float D(const Vec3& wh, const Vec2& alpha) {
        float tan2ThetaV = tan2Theta(wh);
        if(isinf(tan2ThetaV))
            return 0.0f;
        float cos4Theta = sqr(cos2Theta(wh));
        float e = (cos2Phi(wh) / sqr(alpha.x) + sin2Phi(wh) / sqr(alpha.y)) *
            tan2ThetaV;
        return glm::one_over_pi<float>() /
            (alpha.x * alpha.y * cos4Theta * sqr(1.0f + e));
    }
    INLINEDEVICE float lambda(const Vec3& w, const Vec2& alpha) {
        float absTanTheta = fabsf(tanTheta(w));
        if(isinf(absTanTheta))
            return 0.0f;
        // Compute alpha for direction w
        float alphaW =
            sqrtf(cos2Phi(w) * sqr(alpha.x) + sin2Phi(w) * sqr(alpha.y));
        float alpha2Tan2Theta = sqr(alphaW * absTanTheta);
        return (-1.0f + sqrtf(1.0f + alpha2Tan2Theta)) * 0.5f;
    }
    INLINEDEVICE Vec2 trowbridgeReitzSample11(float cosTheta, float U1,
                                              float U2) {
    ...
    INLINEDEVICE Vec3 trowbridgeReitzSample(const Vec3& wi, const Vec2& alpha,
                                            const Vec2& u) {
        // 1. stretch wi
        Vec3 wiStretched =
            normalize(Vec3(alpha.x * wi.x, alpha.y * wi.y, wi.z));
        ...
        // 5. compute normal
        return normalize(Vec3(-slope.x, -slope.y, 1.0f));
    }

    INLINEDEVICE Vec3 sampleWh(const Vec3& wo, const Vec2& alpha,
                               const Vec2& u) {
        Vec3 wh = trowbridgeReitzSample(wo.z >= 0.0f ? wo : -wo, alpha, u);
        return wo.z >= 0.0f ? wh : -wh;
    }

    INLINEDEVICE float pdf(const Vec3& wo, const Vec3& wh, const Vec2& alpha) {
        float G1 = 1.0f / (1.0f + lambda(wo, alpha));
        return D(wh, alpha) * G1 * fabsf(dot(wo, wh)) / absCosTheta(wo);
    }
    INLINEDEVICE float G(const Vec3& wo, const Vec3& wi, const Vec2& alpha) {
