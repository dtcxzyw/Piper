...
public:
    ...
    DEVICE Spectrum f(const Vec3& wo, const Vec3& wi) const {
        switch(mType) {
            case Type::BxDFLambertianReflection:
                return mContent.lr.f(wo, wi);
            case Type::BxDFMicrofacetReflection:
                return mContent.mr.f(wo, wi);
        }
    }
    DEVICE Vec3 sampleF(const Vec3& wo, const Vec2& u) const {
        switch(mType) {
            case Type::BxDFLambertianReflection:
                return mContent.lr.sampleF(wo, u);
            case Type::BxDFMicrofacetReflection:
                return mContent.mr.sampleF(wo, u);
        }
    }
    DEVICE float pdf(const Vec3& wo, const Vec3& wi) const {
        switch(mType) {
            case Type::BxDFLambertianReflection:
                return mContent.lr.pdf(wo, wi);
            case Type::BxDFMicrofacetReflection:
                return mContent.mr.pdf(wo, wi);
        }
    }
};
