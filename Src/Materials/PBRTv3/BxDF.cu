...
DEVICE Spectrum MicrofacetReflection::f(const Vec3 &wo, const Vec3 &wi) const {
    ...
    if (fminf(cosThetaI, cosThetaO) < eps) return black;
    if (fmaxf(wh) < eps) return black;
    wh = normalize(wh);
    Spectrum F = mFresnel.eval(dot(wi, wh));
    return mR * F * (TrowbridgeReitzDistribution::D(wh, mAlpha) *
...
DEVICE float MicrofacetReflection::pdf(const Vec3 &wo, const Vec3 &wi) const {
    ...
    return TrowbridgeReitzDistribution::pdf(wo, wh, mAlpha) / (4.0f * dot(wo, wh));
}
