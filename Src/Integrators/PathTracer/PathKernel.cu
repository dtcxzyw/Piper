...
RT_PROGRAM void traceKernel() {
    ...
    uint32 id =
        filmSize.x * (driverIndex * filmSize.y + pixelPos.y) + pixelPos.x;
    for(uint32 i = 0; i < integratorSample; ++i) {
        ...
        optix::Ray curRay;
        {
            ...
            curRay = optix::make_Ray(sample.ori, sample.dir, 0, 0.0f,
                                     RT_DEFAULT_MAX);
        }
        ...
        for(uint32 i = 0; i < integratorMaxDepth; ++i) {
            ...
            payload.f = black;
            payload.rad = black;
            rtTrace(globalTopNode, curRay, payload,
                    geometryMask | lightVolumeMask);
            seed = payload.index;
            res += att * payload.rad;
            att *= payload.f;
            if(!payload.hit)
